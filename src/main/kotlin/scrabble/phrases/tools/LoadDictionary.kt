...
fun main() {
    ...
    try {
        ...
        println("Restoring rarity levels...")
        conn.createStatement().use {
            it.execute("""
                UPDATE words w SET rarity_level = rb.rarity_level
                FROM rarity_backup rb
                WHERE w.word = rb.word AND w.type = rb.type
